...
const JUNGLE_CAMPS = [
  { name: '블루 두꺼비', x: 16, y: 39, size: 7 },
  { name: '레드 돌거북', x: 42, y: 16, size: 7 },
  { name: '블루 블루', x: 25, y: 43, size: 10 },
  { name: '레드 늑대', x: 70, y: 41, size: 7 },
  { name: '레드 두꺼비', x: 80, y: 53, size: 7 },
  { name: '레드 블루', x: 70, y: 50, size: 10 },
  { name: '블루 레드', x: 50, y: 68, size: 10 },
  { name: '블루 돌거북', x: 55, y: 78, size: 7 },
  { name: '블루 늑대', x: 25, y: 53, size: 7 },
  { name: '레드 레드', x: 46, y: 23, size: 10 },
];
...
function tickAdvance(prev) {
  ...
  if (isFinalTick) {
    ...
    const nexusLoserSide = finalWin ? 'ai' : 'user'; // 넥서스가 뚫리는(패배) 진영
    const nexusWinnerSide = nexusLoserSide === 'user' ? 'ai' : 'user'; // 파괴한(승리) 진영 - objectives[side]는 "그 side가 파괴한 개수"이므로 승리 진영 쪽에 채워야 한다
    if (objectives[nexusWinnerSide].nexusTowers < 2) {
      LANES.forEach((l) => { objectives[nexusWinnerSide].laneTowers[l] = 3; });
      objectives[nexusWinnerSide].nexusTowers = 2;
    }
