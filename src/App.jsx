        ...
        if (res && res.value) {
          ...
          parsed.players = parsed.players.map((p) => {
            ...
            if (!updated.achievements) {
              needsBackfillSave = true;
              updated = { ...updated, achievements: { regionalWins: 0, regionalRunnerUps: 0, internationalWins: 0, internationalRunnerUps: 0 } };
            }
            return updated;
          });
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          {roster.map((p) => (
            <div key={p.id} className={`${panel} p-3`}>
              ...
              <button onClick={() => { setViewingOpponentPlayer(p); setScreen('opponentPlayerDetail'); }} className={`${btnGhost} w-full py-1.5 text-xs`}>상세보기</button>
            </div>
          ))}
        </div>
  ...
  function renderOpponentPlayerDetail() {
    ...
    return (
      <div className="max-w-2xl mx-auto p-4 md:p-8 relative">
        ...
        <button onClick={() => setScreen('clubDetail')} className={`${btnGhost} px-4 py-2 text-sm mb-4`}>← 뒤로</button>
