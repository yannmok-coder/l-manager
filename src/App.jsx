    ...
    const headerNode = (
      <div className="mb-2">
        ...
        <div className="flex items-center justify-center gap-1 flex-wrap text-sm mt-2 px-2 py-1.5 rounded-lg" style={{ border: '1px solid #2A3550' }}>
          {Array.from({ length: LANES.reduce((sum, l) => sum + s.objectives.user.laneTowers[l], 0) + s.objectives.user.nexusTowers }).map((_, i) => <span key={'ut' + i}>🗼</span>)}
          ...
          {Array.from({ length: LANES.reduce((sum, l) => sum + s.objectives.ai.laneTowers[l], 0) + s.objectives.ai.nexusTowers }).map((_, i) => <span key={'at' + i}>🗼</span>)}
