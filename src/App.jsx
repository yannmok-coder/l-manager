...
// 배포판 전용: 바론/드래곤 유형별 아이콘 이미지 (public/objective-icons/)
  ...
  function renderSim() {
    ...
    const headerNode = (
      <div className="mb-2">
        ...
        <table className="w-full text-xs mt-2 rounded-lg overflow-hidden" style={{ border: '1px solid #2A3550' }}>
          <thead>
            <tr className="lm-muted" style={{ background: '#141A2C' }}>
              <th className="text-left px-2 py-1 font-normal"></th>
              <th className="text-center px-2 py-1 font-normal" style={{ color: '#38BDF8' }}>아군</th>
              <th className="text-center px-2 py-1 font-normal" style={{ color: '#EF4444' }}>상대</th>
            </tr>
          </thead>
          <tbody>
            <tr style={{ borderTop: '1px solid #2A3550' }}>
              <td className="px-2 py-1">🗼 타워</td>
              <td className="text-center">{LANES.reduce((sum, l) => sum + s.objectives.user.laneTowers[l], 0) + s.objectives.user.nexusTowers}</td>
              <td className="text-center">{LANES.reduce((sum, l) => sum + s.objectives.ai.laneTowers[l], 0) + s.objectives.ai.nexusTowers}</td>
            </tr>
            <tr style={{ borderTop: '1px solid #2A3550' }}>
              <td className="px-2 py-1">🐉 드래곤</td>
              <td className="text-center"><span className="inline-flex gap-0.5 items-center justify-center">{s.objectives.user.dragons.map((d, i) => <img key={i} src={DRAGON_ICON[d] || DRAGON_ICON.화염} alt={d} title={d} width="14" height="14" />)}</span></td>
              <td className="text-center"><span className="inline-flex gap-0.5 items-center justify-center">{s.objectives.ai.dragons.map((d, i) => <img key={i} src={DRAGON_ICON[d] || DRAGON_ICON.화염} alt={d} title={d} width="14" height="14" />)}</span></td>
            </tr>
            <tr style={{ borderTop: '1px solid #2A3550' }}>
              <td className="px-2 py-1">💀 바론</td>
              <td className="text-center">{s.objectives.user.barons}</td>
              <td className="text-center">{s.objectives.ai.barons}</td>
            </tr>
          </tbody>
        </table>
        {s.elderBuff && (
