...
const CHAMPION_ICON = {
  ...
  '노커': '/champion-icons/champ-nocturne.png',
...
const BLUE_TOWERS = [
  { x: 10, y: 28 }, { x: 14, y: 54 }, { x: 10, y: 68 },
  { x: 44, y: 56 }, { x: 38, y: 70 }, { x: 28, y: 76 },
  { x: 74, y: 92 }, { x: 53, y: 90 }, { x: 34, y: 88 },
  { x: 20, y: 85 }, { x: 14, y: 78 },
];
const RED_TOWERS = [
...
function computePositions(userLineup, aiLineup, eventParticipants, clashPoint, tick, pendingClash, objectives) {
  ...
  allPlayers.forEach(({ key, p, side }) => {
    ...
    if (tick != null && respawnAt > 0 && tick - respawnAt < RESPAWN_WALK_TICKS) {
      // 리스폰 직후: 우물에서 시작해 라인으로 서서히 복귀한다
      const progress = clamp((tick - respawnAt) / RESPAWN_WALK_TICKS, 0, 1);
...
function tickAdvance(prev) {
  ...
  function applyTowerPush(plan) {
    ...
    if (plan.isTwin) so.nexusTowers += 1; else so.laneTowers[plan.lane] += 1;
  ...
  if (pendingClash && pendingClash.phase === 'travel' && tick < pendingClash.arriveTick) {
    ...
    if (pendingClash.kind === 'fight' && pendingClash.combatPlan) {
      ...
      log = [{ id: tick + '-' + Math.random(), text: `${pendingClash.towerPlan.side === 'user' ? '우리 팀' : '상대 팀'}이(가) ${pendingClash.towerPlan.label}을(를) 파괴했습니다!` }, ...log].slice(0, 6);
    }
    pendingClash = null;
  } else {
    ...
    if (roll < (acc += skirmishChance)) {
      ...
      if (selection) {
        ...
        pendingClash = { kind: 'fight', phase: 'travel', selection, targetPoint: pickZone(tickRatio, objectives), userKeys, aiKeys, startTick: tick, arriveTick: tick + ARRIVE_TICKS };
      ...
      if (towerPlan) {
        ...
        if (attackerAvail.length > 0) {
          ...
          pendingClash = {
            kind: 'tower', phase: 'travel', towerPlan, targetPoint: towerPlan.targetPoint,
            userKeys: towerPlan.side === 'user' ? atkKeys : [], aiKeys: towerPlan.side === 'ai' ? atkKeys : [],
            startTick: tick, arriveTick: tick + ARRIVE_TICKS,
          };
          eventParticipants = [...pendingClash.userKeys, ...pendingClash.aiKeys];
        }
      }
