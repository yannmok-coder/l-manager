...
function tickAdvance(prev) {
  ...
  if (prev.endingSequence) {
    ...
    const participantKeys = winnerLineup.map((_, i) => `${es.winnerSide}-${i}`);
    eventParticipants = participantKeys;

    if (tick < es.arriveTick) {
      // 상대 진영으로 진격하는 중 - 서서히 이동한다
      const pc = {
        userKeys: es.winnerSide === 'user' ? participantKeys : [], aiKeys: es.winnerSide === 'ai' ? participantKeys : [],
    ...
    if (tick < es.endTick) {
      // 공성 중: 이번 틱에 한 가지 행동(챔피언 처치 또는 안쪽 타워 파괴)을 진행한다
      const winnerLabel = es.winnerSide === 'user' ? '우리 팀' : '상대 팀';
      const remainingTargets = loserLineup.map((p, i) => i).filter((i) => !loserLineup[i].finaleEliminated);
      const lo = objectives[es.loserSide];
      const openLane = LANES.find((l) => lo.laneTowers[l] < 3);
      if (remainingTargets.length > 0 && Math.random() < 0.55) {
        const victimIdx = remainingTargets[randRange(0, remainingTargets.length - 1)];
        ...
        lo.laneTowers[openLane] += 1;
        objectives.lastTowerTick = tick;
        if (es.winnerSide === 'user') userScore += 3; else aiScore += 3;
        const laneLabel = { top: '탑', mid: '미드', bot: '봇' }[openLane];
        log = [{ id: tick + '-' + Math.random(), text: `${winnerLabel}이(가) ${laneLabel} ${lo.laneTowers[openLane]}차 타워를 파괴했습니다!` }, ...log].slice(0, 6);
      }
      const positions = finalSiegePositions(userLineup, aiLineup, es.targetPoint);
      return { ...prev, tick, userLineup, aiLineup, userScore, aiScore, log, positions, finished: false, eventParticipants, objectives, elderBuff, capBurstUsed, pendingClash: null, endingSequence: es };
    }

    // 공성 종료 - 쌍둥이 타워를 무너뜨리고 넥서스를 파괴한다
    const lo = objectives[es.loserSide];
    LANES.forEach((l) => { lo.laneTowers[l] = 3; });
    lo.nexusTowers = 2;
    objectives.nexusDestroyed = es.loserSide;
    const winnerLabel = es.winnerSide === 'user' ? '우리 팀' : '상대 팀';
    log = [{ id: tick + '-nexus', text: `${winnerLabel}이(가) 쌍둥이 타워를 무너뜨리고 넥서스를 파괴했습니다! 경기 종료!` }, ...log].slice(0, 6);
    const positions = finalSiegePositions(userLineup, aiLineup, es.targetPoint);
  ...
  if (isFinalTick) {
    ...
    const siegeTicks = randRange(6, 12); // 경기시간 기준 30초~1분
    const arriveTick = tick + travelTicks;
    const endTick = arriveTick + siegeTicks;
    const endingSequence = { winnerSide, loserSide, targetPoint, startTick: tick, arriveTick, endTick, finalWin };
  ...
  const baronChance = (baronAvailable && totalBarons < 2) ? (0.15 / TICKS_PER_MIN) * objectiveRushMult : 0;
  ...
  if (pendingClash && pendingClash.phase === 'travel' && tick < pendingClash.arriveTick) {
    ...
    [...pendingClash.userKeys, ...pendingClash.aiKeys].forEach((key) => {
    ...
    pendingClash = { ...pendingClash, phase: 'return', startTick: tick, arriveTick: tick + returnTicks };
      ...
      <div className="mb-2">
        ...
        <div className="flex items-stretch text-sm mt-2 px-2 py-1.5 rounded-lg" style={{ border: '1px solid #2A3550' }}>
          <div className="flex-1 flex items-center justify-end gap-1 flex-wrap">
            {Array.from({ length: s.objectives.user.barons }).map((_, i) => <img key={'ub' + i} src={BARON_ICON} alt="바론" width="16" height="16" />)}
          ...
          <div className="flex-1 flex items-center justify-start gap-1 flex-wrap">
            ...
            {Array.from({ length: s.objectives.ai.barons }).map((_, i) => <img key={'ab' + i} src={BARON_ICON} alt="바론" width="16" height="16" />)}
          </div>
        </div>
        {s.elderBuff && (
